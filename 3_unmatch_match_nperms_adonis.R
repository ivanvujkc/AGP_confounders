...
# calculate scaling-adjustment factor for each metadata variable that differs between cases and controls, based on extent to which they differed (log20 Benjamini-Hochberg Q value divided by nroot of total variables to match for)
...
# implement scaling-adjustment on centered, scaled metadata variables
  ...
  if(matchstat[j]=="unmatched")
	  {
	    ...
	    tempmap<-map3[sample(allnonnas,size=length(allnonnas)),c(locvars, varsthatdiffer)]
	  }
